...
- Produkční tvar Workeru podle ADR-019, ověřený proti lokálnímu Worker runtimu. Dávkové čtení srazilo bootstrap z 661 D1 dotazů na 38 (konstantně, nezávisle na objemu dat) a odezvu z ~300 ms na ~38 ms. Identita jde z `Authorization: Bearer` proti serverovému tajemství — podvržená hlavička `oai-authenticated-user-id` vrací 401. CORS a preflight fungují pro povolený origin a mlčí pro cizí. Worker RSS nestahuje: `pnpm ingest:once` posbíral šest českých zdrojů lokálně a přes `POST /ingest` zapsal 44 nových položek. Produkce běží na <https://siftera-api.jiri-bubnik.workers.dev> (Worker `siftera-api`, D1 `siftera-prod`, R2 `siftera-content-prod`). Ověřeno na živé adrese: bez tokenu i s podvrženou hlavičkou `oai-authenticated-user-id` vrací 401, se správným tokenem 200, CORS odpovídá jen povolenému originu. Lokální `ingest:once` naplnil produkci 125 položkami ze šesti zdrojů a redakční import publikoval vydání o deseti položkách. Nasazení odhalilo dvě chyby, obě opravené: klient si při same-origin běhu myslel, že token nepotřebuje, a service worker po nasazení nové verze držel starou cache, takže prohlížeč dostal místo skriptu HTML.

Dřívější stav: produkce připravená jako prostředí `prod` ve `wrangler.jsonc` (jméno `siftera-api`, vlastní D1/R2 a `ALLOWED_ORIGINS`); `wrangler deploy --dry-run --env prod` prochází. Vlastní nasazení zatím neproběhlo: `wrangler login` je interaktivní a `--temporary` nepřipadá v úvahu, protože dočasné účty nepodporují R2.
...
- Redakční běh 7. 9.: z 80 kandidátů vydáno 25 položek, 55 odmítnuto s konkrétním důvodem, duplicity k AfD, útoku na Actonovou a přejmenování Nového Mexika svázané přes `relatedCandidates`. Devět originálů doplněno přes `/editor/enrich` (tři po třech, aby se vešly do CPU limitu); pět vyšlo jako plný text a nese `basis=full_text`, dva zdroje originál nevydaly. YouTube položky byly nasbírané ještě konektorem bez čtení `media:group`, takže v produkci ležely bez popisu i náhledu — cílený reingest přes `SIFTERA_ONLY` je opravil (30 revizí) a kanál Video má z čeho psát. Po vydání drží proud 36 nepřečtených: News 14, Tech 12, Věda 7, Podcasty 4, Video 4. Pozor na velikost bootstrapu: s knihovnou o 55 položkách vyrostl na 247 kB (odpověď 0,53 s, bez 1102), takže hranice z ADR-020 se blíží znovu a bude potřeba stránkovat i knihovnu.
- Přehrávání videa ve feedu podle ADR-022: YouTube karta má fasádu s náhledem a tlačítkem přehrát; iframe na `youtube-nocookie.com` se vloží až po kliknutí, takže do té doby na přehrávač nejde žádný request (ověřeno v prohlížeči: před kliknutím nula iframů, jen náhled z ytimg). Hraje nejvýš jedno video, spuštění značí položku jako přečtenou a nikam nenaviguje. Konektor plní `medium=video` a `media` (`yt:videoId` i odvození z adresy, délku z `media:content @duration` — YouTube ji ve feedu neposílá), `POST /ingest` obojí propouští; publikovaným položkám bez `media` odvodí klient identifikátor z kanonické adresy a přijme jen platný jedenáctiznakový tvar.
- Kanály podle ADR-021: horní lišta nese ikony Přehled · News · Tech · Podcasty · Video · Věda · Uložené (kategorie z preferencí, ikona z uzavřeného seznamu), režim Feed / Vše je uvnitř kanálu a panel filtruje jen zdroje aktivního kanálu. Karta ukazuje tagy rovnou, „Zobrazit víc" pokračuje v textu a ukáže se jen při skutečném přetečení; „proč to vidím" a podklad jsou v nabídce pod „···". Produkční preference mají pět kanálů nad 24 zdroji (News 8, Tech 5, Podcasty 8, Video 2, Věda 1) a žádný zdroj nezůstal bez kanálu. Service worker se registruje relativně k `BASE_URL`, takže na Pages v podcestě `/siftera/` konečně platí (dřív mířil na neexistující `/sw.js`).
- Jeden feed podle ADR-016: ikona filtrů v hlavičce, která uhýbá při rolování; spodní lišta zrušena, hledání je lupa. Postupné dočítání po 15 položkách, poslední bootstrap v `sessionStorage` pro okamžitý návrat z originálu včetně pohledu a hloubky scrollu. Jméno zdroje se řeší z aktuální konfigurace a zkracuje na jeden řádek. Řazení a kategorie v panelu, vkládání pod „+“. Kategorie jsou uživatelské filtry nad tématy a zdroji uložené v preferencích; Inbox jako samostatná obrazovka zanikl. Horní lišta uhýbá při rolování dolů.
...
Regresní průchod 6. 9. po produkčním nasazení: typecheck, lint, 78 testů v 10 souborech, buildy balíčků, hosted i Pages. Doplněny testy pro to, co dosud pokrytí nemělo — cross-origin přístup (preflight, CORS na chybové odpovědi, odmítnutí cizího originu), dávkový ingest včetně dedupu a odmítnutí neplatné dávky, proud nepřečteného (řazení relevancí s útlumem stáří, okno, degradace viděného, idempotence `markSeen`) a strážce proti návratu N+1: desetinásobek dat nesmí znamenat desetinásobek čtení z repozitáře. V prohlížeči ověřeno ovládání v hlavičce, čtyři varianty karet, dočítání po 15 (15 → 75), filtry, řazení, hledání, rozbalení bez přeskládání okolí a obnova pozice po návratu z originálu. Obě nasazené adresy odpovídají.
